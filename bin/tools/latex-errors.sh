...
#---------------------------------------------------------------------
# Secondly, we need to find `basename'.
#
if test -z "$PATH"
then
  test -f ./gbasename && basename_path=./gbasename
else
  IFS=':'
  for dir in $PATH; do
    test -z "$dir" && dir=.
    if test -f $dir/gbasename; then basename_path=$dir/gbasename; break; fi
  done
fi

if test -z "$basename_path"
then
  if test -z "$PATH"
  then
    test -f ./basename && basename_path=./basename
  else
    IFS=':'
    for dir in $PATH; do
      test -z "$dir" && dir=.
      if test -f $dir/basename; then basename_path=$dir/basename; break; fi
    done
  fi
  test -z "$basename_path" && exit 1
fi

script=$($basename_path $0)
...
function search_command {
  ...
  then
    ...
    if test -n "$current_path"
    then
      eval "$($basename_path $1 .sh)_path=$current_path"
      test "$debug" = "yes" && print_newline "$current_path"
    else
      test "$debug" = "yes" && print_newline " not found."
      fatal_error "Command \"$1\" not found."
    fi
  fi
}

required_commands='rm cat cp cut expr head grep ls nl perl sed sort tail tr uniq wc'
...
then
  if test "$debug" = "yes"
  then
    print_newline "checking for echo... $echo_path"
    print_newline "checking for basename... $basename_path"
  fi
  IFS=" "
...
function usage {
  $cat_path <<EOF
Usage: $($basename_path $0) [-h][-d] <basename>
  ...
  case ${1} in
    ...
    *) if $grep_path 'I found no \\bibstyle command' $blg >/dev/null 2>&1;
       ...
       then
         ...
         $echo_path "      (This error is counted twice.)"
...
function hbox_init {
  ...
  line_in_log=$($sed_path --quiet "$linenum_in_log p" ${1}.log)
...
function SET_message {
  from=$($expr_path $linenum_in_log + 1)
  to=$($expr_path $linenum_in_log + 10)
  chunk="$($sed_path --quiet "$from,$to p" ${1}.log)"
  message_size=$($echo_path $chunk \
  ...
  then
    ...
    message=$($echo_path "$chunk" | $head_path --lines=$message_size)
  fi
}
...
function slide_overfull {
  ...
  line_in_log=$($sed_path --quiet "$linenum_in_log p" ${1}.log)
  page=$($echo_path "$line_in_log" \
          | $sed_path --quiet 's|.*Slide \([0-9]\+\).*|\1|p')  
